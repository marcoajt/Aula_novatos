...
from aula_novatos.exercice_43 import meetup
...
def test_when_first_friday_is_the_7th_the_last_day_of_the_first_week():
    assert meetup(2012, 12, "first", "Friday") == date(2012, 12, 7)


# Additional tests for this track

def test_fifth_monday_of_march_2015():
    assert meetup(2015, 3, "fifth", "Monday") == date(2015, 3, 30)


def test_fifth_thursday_of_february_2024():
    assert meetup(2024, 2, "fifth", "Thursday") == date(2024, 2, 29)


def test_fifth_saturday_of_february_2020():
    assert meetup(2020, 2, "fifth", "Saturday") == date(2020, 2, 29)


def test_last_sunday_of_june_2024():
    assert meetup(2024, 6, "last", "Sunday") == date(2024, 6, 30)


def test_teenth_friday_of_may_2022():
    assert meetup(2022, 5, "teenth", "Friday") == date(2022, 5, 13)


def test_nonexistent_fifth_monday_of_february_2022():
    with pytest.raises(MeetupDayException) as err:
        meetup(2022, 2, "fifth", "Monday")
    assert type(err.value) is MeetupDayException
    assert err.value.args[0] == "That day does not exist."


def test_nonexistent_fifth_friday_of_august_2022():
    with pytest.raises(MeetupDayException) as err:
...
def test_nonexistent_fifth_thursday_of_may_2023():
    with pytest.raises(MeetupDayException) as err:
